...
RSpec.describe Tasks::WordGames do
  describe '#items' do
    let(:task) { build(:word_games_task) }

    it 'is built' do
      expect(task.items.map(&:game_type)).to eq(TaskItems::WordGames::GAME_TYPES)
    end

    it 'is enabled' do
      expect(task.items).to all(be_enabled)
    end
  end
end
